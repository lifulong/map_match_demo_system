...
class source {
	...
	public function __construct($filename){
		...
		$splits=explode("\n", $content);
		foreach($splits as $roads)
		{
			$json_roads = json_decode($roads, True);
			$this->roads[] = $json_roads;;
		}
		echo "<script type='text/javascript'>console.log('source:".count($this->roads)."')</script>";
	}
